 ...
 * @return array
 */
function flg_save_gift($name,$sender,$receiver,$message, $image_url, $flg_id) {
  ...
  if ($guid == 0) {
	  $gift = new ElggObject;
	  $gift->subtype = "gifts";
	  $gift->container_guid = elgg_get_logged_in_user_guid();
	  system_message('new');
  } else {
  ...
  if ($new) {
	  $r=add_to_river('river/gifts_create','create',$gift->sender , $gift->getGUID());
  }
